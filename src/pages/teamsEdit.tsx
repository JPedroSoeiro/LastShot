...
function TeamsEdit() {
  ...
  // Carregar os dados do time quando a página for carregada
  useEffect(() => {
    if (id) {
      const fetchTeamData = async () => {
        const teamData = await getTeamById(Number(id)); // Pega o time pelo id
        if (teamData) {
          setTeam(teamData);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Chama a função de atualização do time
      await updateTeam(Number(id), team); // Atualiza o time com base no id
      setIsLoading(false);
      navigate("/times"); // Navega para a página de times após a atualização bem-sucedida
    } catch (error) {
      setIsLoading(false);
